...
CMoving::CMoving(CCI_QuadRotorPositionActuator* pcPropellers, CCI_PositioningSensor* pcPos) {
...
void CMoving::GoInSpecifiedDirection(SensorSide sensorSide) {
    ...
    switch (sensorSide) {
        ...
        case SensorSide::kLeft:
            xVector = cos(PI_OVER_FOUR - currentAngle->GetValue()) * - SPEED_SIDE;
            yVector = sin(PI_OVER_FOUR - currentAngle->GetValue()) * SPEED_SIDE;
            break;
        ...
        case SensorSide::kBack:
            xVector = cos(currentAngle->GetValue() + PI_OVER_FOUR) * SPEED_SIDE;
            yVector = sin(currentAngle->GetValue() + PI_OVER_FOUR) * SPEED_SIDE;
            break;
        default:
            break;        
    }
