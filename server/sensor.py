...
# when all 3 angular values are null :
    ...
    def __init__(self, u,d,l,ri,f,b,y,p,ro):
        self.up    = u
        self.down  = d
        self.left  = l
        self.right = ri
    ...
    def getEdgeRight(self)-> Vec3 :
        coord = Vec3(0,0,0)
        coord.x = self.left * math.cos(self.roll) * math.sin(self.yaw)
        coord.y = self.left * math.cos(self.roll) * math.cos(self.yaw)
        coord.z = self.left * math.sin(self.roll)
        return coord
    ...
    def toJson(self) :
        ...
        'pitch' : self.pitch ,
        'roll ' : self.roll  ,
        'detectedR' : getEdgeRight().toJson(),
        'detectedL' : getEdgeLeft().toJson(),
        'detectedF' : getEdgeFront().toJson()
        }
